 ...
 * 
 * @param i, si carácter recibido es signo de puntuación devuelve 1, sino devuelve 0
 * @return int 
 */
int eraseSign(int i){
     if( (i > 32 && i < 48)  || ( i >= 58 && i <= 64)) return 1;
return 0;
}
 ...
 * 
 * @param file[] 
 * @return int , si tuvo existo devuelve el descriptor, sino devuelve EXIT_FAILURE.
 */
 ...
 * @return int, si puede escribir en el fichero devuelve EXIT_SUCCESS, sino EXIT_FAILURE
 */
...
/**
 * @brief Comprueba si puede leer un fichero.
 * 
 * @param f, descriptor del archivo
 * @param buffer, devuelte cadena leída
 * @param length, tamaño de caracteres a leer  
 * @return int, si puede leer el fichero devuelve EXIT_SUCCESS, sino EXIT_FAILURE 
 */
...
/**
 * @brief función recursiva que leer cada cadena del fichero y lo devuelve al @file main.c
 * 
...
int busqueda(int file, char* string ){
    ...
    if( !strcmp(string,buffer) )return 1;
    ...
    if( i == f) return out;
            ...
            if(!save){ 
                    ...
                    if( (out = (char**)realloc(out,sizeof(char*)*(*j+1))) == NULL){
                            ...
                            for (int i = 0; i < (*j+1); i++) out[i] = (char*)realloc(out[i],sizeof(char)*(strlen(buffer)+1));   
                    }

                    if( out[*j] == NULL)perror("out:");
...
/**
 * @brief Comrpeuba si fichero @param f es f.txt 
 * 
...
/**
 * @brief Interfaz de usuario en el que elige el documento a visualizar y su forma normalizada.
 * 
